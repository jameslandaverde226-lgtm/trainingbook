...
import { 
  ...
  Check, Trash2, Edit3, Save, ChevronRight, LayoutGrid
} from "lucide-react";
...
export default function CertificationsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] relative font-sans select-none overflow-x-hidden pt-6 pb-32 md:pb-0">
      ...
      {/* --- FLOATING VIEW SWITCHER (Tabs) --- */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] md:absolute md:top-8 md:left-auto md:right-0 md:translate-x-0 md:bottom-auto">
          <div className="flex p-1 bg-white/90 backdrop-blur-xl border border-white/60 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] rounded-full ring-1 ring-black/5">
              <button 
                  ...
                  className={cn(
                      ...
                      activeView === "armory" ? "bg-slate-900 text-white shadow-lg" : "text-slate-400 hover:text-slate-600"
                  )}
              >
                  <Award className="w-4 h-4" /> Armory
              </button>
              <button 
                  onClick={() => setActiveView("eotm")}
                  className={cn(
                      "flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all",
                      activeView === "eotm" ? "bg-amber-400 text-black shadow-lg shadow-amber-400/30" : "text-slate-400 hover:text-slate-600"
                  )}
              >
                                                                ...
                                                                <div className="flex flex-wrap gap-1.5 md:gap-2">
                                                                    {badges.map((b: any) => {
                                                                        ...
                                                                        return (
                                                                            ...
                                                                            >
                                                                                <div className="h-7 w-7 md:h-8 md:w-8 rounded-lg flex items-center justify-center shadow-sm border border-white bg-white hover:scale-110 hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden">
      ...
      {/* --- MOBILE FAB (Only show when in Armory view) --- */}
      {activeView === 'armory' && (
        <button 
