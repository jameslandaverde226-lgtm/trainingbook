...
import { 
  motion, AnimatePresence, useDragControls, LayoutGroup, Transition, PanInfo 
} from "framer-motion";
import { 
...
function TrainingDynamicIsland({ 
    ...
    const activeEditorClass = activeDept === "FOH" 
        ? "bg-[#004F71] border-[#004F71] shadow-lg shadow-blue-900/20" 
        : "bg-[#E51636] border-[#E51636] shadow-lg shadow-red-900/20";
                            ...
                            >
                                ...
                                <motion.div layout="position" className="flex flex-col justify-center min-w-[120px]">
                                    ...
                                    <motion.span layoutId="subtitle-text" className="text-[9px] font-bold text-slate-400 whitespace-nowrap truncate max-w-[140px]">
                                        {activePhaseTitle || "Curriculum Loading..."}
                                    </motion.span>
                                </motion.div>

                                <motion.div layoutId="chevron" className="opacity-40 pl-1 border-l border-slate-200 ml-1">
                            ...
                            >
                                <div className="flex items-center justify-between px-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <motion.span layoutId="title-text" className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                                            Configuration
                                        </motion.span>
                                    </div>
                                    <motion.button 
                                            ...
                                            >
                                                {isActive && (
                                                    <motion.div
                                                        ...
                                                        className={cn("absolute inset-0 rounded-[16px] shadow-sm", dept === "FOH" ? "bg-[#004F71] shadow-blue-500/20" : "bg-[#E51636] shadow-red-500/20")}
                                                        transition={SPRING_TRANSITION}
                                                    />
                                                )}
                                                <span className="relative z-10 flex items-center gap-2">
                                                    {dept === "FOH" ? <Coffee className="w-3.5 h-3.5" /> : <Utensils className="w-3.5 h-3.5" />}
                                                    {dept} Training
                                                </span>
                                            </button>
                                        );
                                ...
                                >
                                    <button
                                        ...
                                        className={cn(
                                            "w-full flex items-center justify-between px-4 py-3.5 rounded-2xl border transition-all duration-200 text-left group relative overflow-hidden",
                                            !previewMode
                                                ? cn("text-white", activeEditorClass)
                                                : "bg-white border-slate-100 hover:border-slate-200 text-slate-500 hover:bg-slate-50"
                                        )}
                                    >
                                        <div className="flex items-center gap-3">
                                            {!previewMode ? <Settings className="w-4 h-4 text-white" /> : <Eye className="w-4 h-4" />}
                                            <span className="text-[10px] font-bold uppercase tracking-wider">
                                                {!previewMode ? "Editor Active" : "Preview Mode Active"}
                                            </span>
                                        </div>
                     ...
                     <div className="space-y-2 md:space-y-2.5">
                        ...
                        {section.tasks.map(task => (
                           <motion.div 
                                layout 
                                key={task.id} 
                                className="group bg-slate-50/50 rounded-xl md:rounded-2xl p-3 md:p-4 border border-transparent hover:border-slate-200 hover:bg-white transition-all flex flex-col gap-3" 
                           ...
                           >
                              ...
                              {/* --- GORGEOUS CINEMATIC BANNER WITH BLURRED FILL --- */}
                              {task.image && (
                                <motion.div 
                                ...
                                >
                                    ...
                                    {/* LAYER 2: MAIN IMAGE (Perfectly fitted, nothing cut off) */}
                                    <motion.img 
                                        src={task.image} 
                                    ...
                                    <div className="absolute inset-0 z-20 bg-black/0 group-hover/img:bg-black/20 transition-colors flex items-center justify-center opacity-0 group-hover/img:opacity-100 pointer-events-none">
                                        <div className="bg-white/20 backdrop-blur-md p-2.5 rounded-full text-white border border-white/40 shadow-lg">
                        ...
                        <AnimatePresence>{!previewMode && (<motion.button initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} onClick={(e) => { e.stopPropagation(); const newTask = { id: Math.random().toString(36).substr(2, 9), title: "", duration: "15m" }; updateSection(section.id, { tasks: [...section.tasks, newTask] }); }} className="w-full py-4 md:py-3.5 border-2 border-dashed border-slate-200 rounded-xl md:rounded-2xl text-[10px] font-black uppercase text-slate-400 hover:border-[#004F71] hover:text-[#004F71] transition-all flex items-center justify-center gap-2 group"><Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" /> Add Logic Block</motion.button>)}</AnimatePresence>
                     </div>
            ...
            <div className="sticky top-[160px] z-40 transition-all duration-500">
               ...
               <div className="h-[calc(100vh-220px)] bg-white rounded-[44px] border border-slate-200/60 shadow-2xl overflow-hidden flex flex-col relative ring-1 ring-black/5">
                  <div className="px-7 py-5 border-b flex justify-between items-center bg-white/50 backdrop-blur-md shrink-0">
                     ...
                     <div className="flex items-center gap-3"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Pages {activeSection?.pageStart || "?"}—{activeSection?.pageEnd || "?"}</span><div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]" /></div>
                  </div>
                  ...
                  <div className="px-7 py-4 bg-white/50 border-t border-slate-100 flex justify-between items-center shrink-0"><span className="text-[9px] font-bold uppercase text-slate-300 tracking-widest leading-none">Trainingbook Command v4.0</span><a href={CANVA_LINKS[activeDept] || CANVA_LINKS.FOH} target="_blank" className="flex items-center gap-2 text-[10px] font-black text-blue-600 hover:underline uppercase tracking-widest transition-all"><Maximize2 className="w-3 h-3" /> Fullscreen</a></div>
               </div>
            </div>
         </div>
      </div>

      {/* --- MOBILE MANUAL VIEWER --- */}
